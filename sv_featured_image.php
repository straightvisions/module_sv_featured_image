...
class sv_featured_image extends init {
	public function init() {
		...
		$this->set_module_desc( __( 'This module shows a featured image via "[sv_featured_image]" shortcode.', 'straightvisions_100' ) );

		// Section Info
		$this->set_section_title( 'Featured Image' );
		$this->set_section_desc( __('Settings for Featured Image', 'straightvisions_100') );
	...
	public function load_settings() :sv_featured_image {
		$this->s['fallback_image'] = $this->get_setting()
			...
			->set_title( __( 'Fallback image', 'straightvisions_100' ) )
			->set_description( __( 'Uploaded image will be used when post has not featured image set.', 'straightvisions_100' ) )
			->load_type( 'upload' );
